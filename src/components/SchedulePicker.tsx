...
export default function SchedulePicker({ onSelect, selectedDate, selectedTime }: SchedulePickerProps) {
  ...
  const timeSlots = {
    ...
    Midday: ["12:00 PM", "12:30 PM", "01:00 PM", "03:00 PM", "03:30 PM"],
  ...
  return (
    <div className="space-y-5 sm:space-y-6 w-full text-gray-900 bg-white">
      {/* Date Selection */}
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 min-h-[96px] sm:min-h-[108px] content-start">
          {timeSlots[selectedPeriod as keyof typeof timeSlots].map((time) => (
            <button
              ...
              className={`w-[calc(33.333%-6px)] sm:w-[calc(25%-9px)] py-3 sm:py-3 rounded-xl border text-[12px] sm:text-[14px] font-bold transition-all cursor-pointer ${
                selectedTime === time
